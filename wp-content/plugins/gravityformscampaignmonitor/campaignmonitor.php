...
class GFCampaignMonitor {
    ...
    public static function init(){
    	//supports logging
		add_filter("gform_logging_supported", array("GFCampaignMonitor", "set_logging_supported"));
        ...
        if(is_admin()){
            ...
            add_action("gform_paypal_action_fields", array("GFCampaignMonitor", "add_paypal_settings"), 10, 2);
            add_filter("gform_paypal_save_config", array("GFCampaignMonitor", "save_paypal_settings"));
            ...
            add_filter( 'mwp_premium_update_notification', array('GFCampaignMonitor', 'premium_update_push') );
            add_filter( 'mwp_premium_perform_update', array('GFCampaignMonitor', 'premium_update') );
        }
        ...
        if(self::is_campaignmonitor_page()){
            ...
            add_action("gform_after_submission", array('GFCampaignMonitor', 'export'), 10, 2);

            //handling paypal fulfillment
            add_action("gform_paypal_fulfillment", array("GFCampaignMonitor", "paypal_fulfillment"), 10, 4);
        }
    }
                        ...
                        if(is_array($settings) && sizeof($settings) > 0){
                            ...
                            <tr>
                                <td colspan="5" style="padding:20px;">
                                    <?php printf(__("You don't have any Campaign Monitor feeds configured. Let's go %screate one%s!", "gravityformscampaignmonitor"), '<a href="admin.php?page=gf_campaignmonitor&view=edit&id=0">', "</a>"); ?>
                                </td>
                            </tr>
                            <?php
                        }
                            ...
                            <tr>
                                <td colspan="5" style="padding:20px;">
                                    <?php printf(__("To get started, please configure your %sCampaign Monitor Settings%s.", "gravityformscampaignmonitor"), '<a href="admin.php?page=gf_settings&addon=Campaign+Monitor">', "</a>"); ?>
                                </td>
                            </tr>
                            <?php
                        }
    ...
    public static function edit_page(){
        ...
        <div class="wrap">
            <img alt="<?php _e("Campaign Monitor", "gravityformscampaignmonitor") ?>" style="margin: 15px 7px 0pt 0pt; float: left;" src="<?php echo self::get_base_url() ?>/images/campaignmonitor_wordpress_icon_32.png"/>
            <h2><?php _e("Campaign Monitor Feed", "gravityformscampaignmonitor") ?></h2>
        ...
        if(!self::is_valid_key()){
            ?>
            <div class="error" style="padding:15px;"><?php echo sprintf(__("We are unable to login to Campaign Monitor with the provided API key. Please make sure you have entered a valid API key in the %sSettings Page%s", "gravityformscampaignmonitor"), "<a href='?page=gf_settings&addon=Campaign+Monitor'>", "</a>"); ?></div>
            <?php
            return;
        }
        ...
        <form method="post" action="">
            ...
            <div class="margin_vertical_10">
                ...
                //getting all clients
                self::log_debug("Getting clients");
                $response = $api->get_clients();
                if (!$response->was_successful()){
                    _e("Could not get client list from Campaign Monitor.", "gravityformscampaignmonitor");
                    self::log_error("Could not get client list from Campaign Monitor.");
                }
                else{
                    $clients = $response->response;
                    self::log_debug("Client list retrieved: " . print_r($clients, true));
                            ...
                            <tr>
                                <td>
                                    <div id="campaignmonitor_optin_condition_field_container" <?php echo !rgar($config["meta"],"optin_enabled") ? "style='display:none'" : ""?>>
                                        <div id="campaignmonitor_optin_condition_fields" style="display:none">
                                            <?php _e("Export to Campaign Monitor if ", "gravityformscampaignmonitor") ?>
                                            <select id="campaignmonitor_optin_field_id" name="campaignmonitor_optin_field_id" class='optin_select' onchange='jQuery("#campaignmonitor_optin_value_container").html(GetFieldValues(jQuery(this).val(), "", 20));'></select>
                                            <select id="campaignmonitor_optin_operator" name="campaignmonitor_optin_operator">
                                                ...
                                                <option value=">" <?php echo rgar($config['meta'], 'optin_operator') == ">" ? "selected='selected'" : "" ?>><?php _e("greater than", "gravityformscampaignmonitor") ?></option>
                                                <option value="<" <?php echo rgar($config['meta'], 'optin_operator') == "<" ? "selected='selected'" : "" ?>><?php _e("less than", "gravityformscampaignmonitor") ?></option>
                                                <option value="contains" <?php echo rgar($config['meta'], 'optin_operator') == "contains" ? "selected='selected'" : "" ?>><?php _e("contains", "gravityformscampaignmonitor") ?></option>
                                                <option value="starts_with" <?php echo rgar($config['meta'], 'optin_operator') == "starts_with" ? "selected='selected'" : "" ?>><?php _e("starts with", "gravityformscampaignmonitor") ?></option>
                                                <option value="ends_with" <?php echo rgar($config['meta'], 'optin_operator') == "ends_with" ? "selected='selected'" : "" ?>><?php _e("ends with", "gravityformscampaignmonitor") ?></option>
                                            </select>
                                            <div id="campaignmonitor_optin_value_container" name="campaignmonitor_optin_value_container" style="display:inline;"></div>
                                        </div>
                                        <div id="campaignmonitor_optin_condition_message" style="display:none">
                ...
                <div id="campaignmonitor_options_container" valign="top" class="margin_vertical_10">
                    ...
                    <div id="campaignmonitor_options">
                        <table>
                            <tr>
                                <td>
                                    <input type="checkbox" name="campaignmonitor_resubscribe" id="campaignmonitor_resubscribe" value="1" <?php echo rgar($config["meta"],"resubscribe") ? "checked='checked'" : "" ?> onclick="var element = jQuery('#campaignmonitor_resubscribe_warning'); if(this.checked){element.show('slow');} else{element.hide('slow');}" />

                                    <label for="campaignmonitor_resubscribe">
                                        <?php _e("Resubscribe" , "gravityformscampaignmonitor") ?>
                                        <?php gform_tooltip("campaignmonitor_resubscribe") ?> <br />
                                        <span id='campaignmonitor_resubscribe_warning' <?php echo !rgar($config["meta"],"resubscribe") ? "style='display:none'" : "" ?>>(<?php _e("This option will re-subscribe users that have been unsubscribed. Use with caution and only when appropriate.", "gravityformscampaignmonitor") ?>)</span>
                                    </label>
        ...
        <script type="text/javascript">
            ...
            function SetOptin(selectedField, selectedValue){
                ...
                if(optinConditionField){
                    ...
                    jQuery("#campaignmonitor_optin_value_container").html(GetFieldValues(optinConditionField, selectedValue, 20));
					jQuery("#campaignmonitor_optin_value").val(selectedValue);
                }
                else{
            ...
            function GetFieldValues(fieldId, selectedValue, labelMaxCharacters){
                ...
                if(field["type"] == "post_category" && field["displayAllCategories"]){
					str += '<?php $dd = wp_dropdown_categories(array("class"=>"optin_select", "orderby"=> "name", "id"=> "campaignmonitor_optin_value", "name"=> "campaignmonitor_optin_value", "hierarchical"=>true, "hide_empty"=>0, "echo"=>false)); echo str_replace("\n","", str_replace("'","\\'",$dd)); ?>';
				}
				else if(field.choices){
					str += '<select id="campaignmonitor_optin_value" name="campaignmonitor_optin_value" class="optin_select">'
				...
				else
				{
					...
					str += "<input type='text' placeholder='<?php _e("Enter value", "gravityforms"); ?>' id='campaignmonitor_optin_value' name='campaignmonitor_optin_value' value='" + selectedValue.replace(/'/g, "&#039;") + "'>";
				}
    ...
    public static function get_lists($client_id, $list_id = ""){
        ...
        if(!$response->was_successful())
        {
			self::log_error("Could not get lists from Campaign Monitor.");
			return $str;
        }


        $lists = $response->response;
        self::log_debug("Lists retrieved: " . print_r($lists, true));
        if(is_array($lists)){
    ...
    public static function get_mapped_field_list($variable_name, $selected_field, $fields){
        $field_name = "campaignmonitor_map_field_" . esc_attr($variable_name);
    ...
    public static function export($entry, $form, $is_fulfilled = false){
        self::log_debug("Entry {$entry["id"]} - Checking if entry is supposed to be sent to Campain Monitor.");
		...
		if( empty ($feeds) ){
			//Not a Campaign Monitor feed
			return;
		}

        $paypal_config = self::get_paypal_config($form["id"], $entry);

        if($paypal_config && rgar($paypal_config["meta"], "delay_campaignmonitor_subscription") && !$is_fulfilled){
			//if configured to only subscribe users when payment is received, delay subscription until the payment is received.
            self::log_debug("Entry {$entry["id"]} - Configured for delayed subscription with PayPal Standard. Not subscribing user now. Will wait for PayPal IPN.");
            return;
        }
        ...
        self::log_debug("Entry {$entry["id"]} - Campain Monitor feeds: " . print_r($feeds, true));
        ...
        foreach($feeds as $feed){
            ...
            if(self::is_optin($form, $feed, $entry)){
                ...
                //updating meta to indicate this entry has already been subscribed to Campaign Monitor. This will be used to prevent duplicate subscriptions.
        		gform_update_meta($entry["id"], "campaignmonitor_is_subscribed", true);
			}
			else{
    ...
    public static function export_feed($entry, $form, $feed){
            ...
            if(GFCommon::is_product_field($field["type"]) && rgar($field, "enablePrice")){
                ...
                foreach($field["inputs"] as $input){
                    ...
                    if(!rgempty($index, $entry)){
                        $merge_vars[] = array("Key" => $var_key, "Value" => apply_filters("gform_campaignmonitor_field_value", rgar($entry, $index), $form["id"], $field_id, $entry));
                    }
                }
            }
            else if(!in_array($var_key, array('email', 'fullname'))){
                $merge_vars[] = array("Key" => $var_key, "Value" => apply_filters("gform_campaignmonitor_field_value", rgar($entry, $field_id), $form["id"], $field_id, $entry));
            }
        ...
        if(!$override_custom_fields){
            $merge_vars = self::remove_blank_custom_fields($merge_vars);
        }
		...
		self::log_debug("Preparing subscriber API calls");
        $api = new CS_REST_Subscribers($feed["meta"]["contact_list_id"], self::get_api_key());
        self::log_debug("Response from subscriber prep" . print_r($api, true));
        self::log_debug("Adding subscriber: " . print_r($subscriber, true));
        $add_response = $api->add($subscriber);
        self::log_debug("Response from attempting to add subscriber: " . print_r($add_response, true));
	...
	public static function add_paypal_settings($config, $form) {
		...
		$settings_style = self::has_campaignmonitor(rgar($form, "id")) ? "" : "display:none;";

		$campaignmonitor_feeds = array();
        foreach(GFCampaignMonitorData::get_feeds() as $feed) {
            $campaignmonitor_feeds[] = $feed['form_id'];
        }
        ?>
        <li style="<?php echo $settings_style?>" id="gf_delay_campaignmonitor_subscription_container">
            <input type="checkbox" name="gf_paypal_delay_campaignmonitor_subscription" id="gf_paypal_delay_campaignmonitor_subscription" value="1" <?php echo rgar($config['meta'], 'delay_campaignmonitor_subscription') ? "checked='checked'" : ""?> />
            <label class="inline" for="gf_paypal_delay_campaignmonitor_subscription">
                <?php
                _e("Subscribe user to Campaign Monitor only when payment is received.", "gravityformscampaignmonitor");
                ?>
            </label>
        </li>
        ...
        <script type="text/javascript">
            jQuery(document).ready(function($){
                jQuery(document).bind('paypalFormSelected', function(event, form) {
                    ...
                    var campaignmonitor_form_ids = <?php echo json_encode($campaignmonitor_feeds); ?>;
                    var has_registration = false;

                    if(jQuery.inArray(String(form['id']), campaignmonitor_form_ids) != -1)
                        has_registration = true;

                    if(has_registration == true) {
                        jQuery("#gf_delay_campaignmonitor_subscription_container").show();
                    } else {
                        jQuery("#gf_delay_campaignmonitor_subscription_container").hide();
                    }
                });
            });
        </script>
    ...
    private static function add_entry_meta($form){
    ...
    public static function save_paypal_settings($config) {
        $config["meta"]["delay_campaignmonitor_subscription"] = rgpost("gf_paypal_delay_campaignmonitor_subscription");
        return $config;
    }
    ...
    public static function paypal_fulfillment($entry, $config, $transaction_id, $amount) {
        ...
        $is_subscribed = gform_get_meta($entry["id"], "campaignmonitor_is_subscribed");
    ...
    public static function has_campaignmonitor($form_id){
        if(!class_exists("GFCampaignMonitorData"))
            require_once(self::get_base_path() . "/data.php");

        //Getting Campaign Monitor settings associated with this form
        $config = GFCampaignMonitorData::get_feed_by_form($form_id);
	...
	{
		$plugins[self::$slug] = "Campaign Monitor";
		return $plugins;
	}
